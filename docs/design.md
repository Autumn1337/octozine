...
**Date**: 2026-05-06
**Authors**: Autumn
**Project name**: `octozine` (octo + zine = 一份独立的 GitHub 杂志)

---

## 1. 摘要

Octozine 是一个**自部署、AI 增强、个性化**的 GitHub 项目发现工具。
...
- 跨源去重 + 历史去重
- 基于多 GitHub 信号的兴趣画像 + YAML 显式偏好（profile / owned repos / public activity / starred / README excerpts）
- LLM 双语摘要（50–80 字 / 项目）
- 静态站（Astro，杂志风）
...
2. **基于用户的个性化** — LLM 读 GitHub 多信号推断兴趣画像，再过滤排序
...
| 3 | **profile** | GitHub 多信号 + `profile.yaml` | profile v2 对象 | 触发生成的条件见 §9；否则直接读 yaml |
| 4 | **rank** | 候选 + profile | top N + reasons + matched signals | 一次 LLM 调用比对画像和候选；产 score(0-100) + reason + matched_themes/languages |
...
1. Action 拉 `github_username` 的 public profile、owned repos、public events、starred repos
2. 从代表性 repo 拉 README excerpt，并用本地统计汇总语言、topic、owner 信号
3. 两次 LLM 调用：extractor 生成画像草稿，critic 去掉空泛主题和 starred 噪声
4. 输出 `profile.yaml`（带 `# generated YYYY-MM-DD` 注释）
5. commit 回仓库（和 issue 一起在 pipeline 的 commit step 里）

`profile.yaml` 结构：
```yaml
# generated 2026-05-07 from yourname's GitHub signals
version: 2
generated_from:
  username: yourname
  generated_at: 2026-05-07
  signals: { owned_repos: 12, starred_repos: 100, activity_repos: 8, readmes: 8 }
core_themes:
  - name: "LLM 工具链与推理引擎"
    weight: 0.92
    confidence: high
  ...
  - name: "终端 TUI 与开发者工具"
    ...
    evidence:
      - source: starred_repo
        ...
        note: "Repeated TUI/devtool signal."
languages:
  - name: rust
    weight: 0.86
    evidence_count: 14
...
LLM 一次性输入：`profile.yaml` + 候选项目列表（每个: owner/repo + description + topics + sources）→ 输出每个项目的 `score(0-100) + reason(中文一句话) + matched_themes + matched_languages`。

reason 会展示在站点上（"为什么推它"），强可解释性。

### 用户编辑

profile.yaml 完全可手编。下次跑直接读手编结果。如果用户想用一个全新的 GitHub 多信号推断，把 `config.profile.regenerate` 改 true，下次跑会重新生成、用完后自动改回 false。
...
# 个性化
profile:
  regenerate: false             # true 时下次重新基于 GitHub 多信号生成
  include: []                    # 显式偏好，优先级最高
  exclude: []                    # 显式排除，优先级最高
  readme_repos: 8
  starred_limit: 150
