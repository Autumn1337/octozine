...
### Step 2 · 在你 fork 后的 repo 里改 `config/config.yaml` 这 3 处
...
profile:
  regenerate: true                          # ← 第一次跑改 true,LLM 会读你的 starred 生成画像,跑完自动翻回 false
```
...
第一次跑时,LLM 读你最近 100 个 starred,生成 `config/profile.yaml`,大概长这样:
...
要 override 默认 model: `model: deepseek-v4-pro`。
要换不在表里的 provider: `provider: custom` + `base_url` + `model`。
