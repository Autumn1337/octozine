...
export async function runPipeline(opts: RunOpts = {}): Promise<IssueData> {
  ...
  const profile = await ensureProfile({
    ...
    explicitInclude: config.profile.include,
    explicitExclude: config.profile.exclude,
    readmeRepos: config.profile.readmeRepos,
    starredLimit: config.profile.starredLimit,
    activityLimit: config.profile.activityLimit,
